...
document.getElementById("bmiCircle").addEventListener("click", function () {
...
document
  .getElementById("exerciseCircle")
  .addEventListener("click", function () {
    let activityLevel = prompt(
      "활동 수준을 입력하세요 (매우 약간, 약간, 보통, 좋음, 매우 좋음):"
    );
    ...
    if (activityLevel) {
      ...
      document.getElementById(
        "exerciseCircle"
      ).innerText = `추천 운동: ${recommendedExercise}`;
    }
  });
document.getElementById("bmrCircle").addEventListener("click", function () {
  ...
  if (gender && !isNaN(weight) && !isNaN(height) && !isNaN(age)) {
    ...
    document.getElementById("bmrCircle").innerText = `BMR: ${bmr.toFixed(
      2
    )} 칼로리`;
  }
});
